...
2026-07-31 审计中，删除了按用户函数/全局变量名称、固定数组长度和特定递归
结构触发的 `TransposeProvenanceLowering` 与 `AffineRecursiveMemoization`。
保留的 `RedundantIterationElimination` 和
`DynamicIdempotentLoopElimination` 只依据 CFG、循环归纳变量、def-use、内存
读写效果及运行时不动点检查触发；测试使用了不同名称、数组规模和迭代次数的等价
程序，确认触发条件不依赖已知测例标识。转换的适用前提、等价性论证及实现仍须由
小组成员在正式提交前逐项复核并能够在答辩中解释。
...
| 2026-07-31 | OpenAI Codex | 实现并验证基于数据流/内存效果的冗余迭代消除与带运行时守卫的不动点循环消除；删除按名称、固定规模和特定递归结构触发的两项优化；增加改名、改规模、改迭代次数的泛化测试；完成 60 个性能用例与 140 个功能用例的本地回归 | 转换前提、等价性及官方平台结果待小组成员复核 |
