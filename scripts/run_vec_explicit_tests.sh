#!/bin/bash
# 《前端+显式长度》 显式长度向量测试运行器（test/functional/vec_explicit/*.sy）
# Usage (in WSL Ubuntu-24.04-eval):
#   bash scripts/run_vec_explicit_tests.sh [O0|O1|o0|o1|o2|o3]
set -e
...
TEST_DIR="${PROJECT_DIR}/test/functional/vec_explicit"
...
echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  显式长度向量测试 (Explicit-length Vec, ${OPT})${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""

mkdir -p /tmp/vec_explicit

pass=0 fail=0
for src in "${TEST_DIR}"/*.sy; do
    [ -f "$src" ] || continue
    name=$(basename "$src" .sy)
    asm="/tmp/vec_explicit/${name}.S"
    bin="/tmp/vec_explicit/${name}_bin"
    cerr="/tmp/vec_explicit/${name}_cerr.txt"
    lerr="/tmp/vec_explicit/${name}_lerr.txt"
