#!/bin/bash
# ================================================================
# SysY Compiler - Full Functional Test Suite (O0)
# ================================================================
export TMPDIR=~/tmp
mkdir -p ~/tmp
...
echo "============================================================"
echo "SysY Compiler - Full Functional Test Suite (O0)"
echo "============================================================"
echo ""
...
for src in "${FUNC_DIR}"/*.sy; do
    ...
    if [ -f "$infile" ]; then
        timeout 5 $QEMU "$bin" < "$infile" > ~/tmp/func_test_${name}_out.txt 2>/dev/null
    else
        timeout 5 $QEMU "$bin" > ~/tmp/func_test_${name}_out.txt 2>/dev/null
    fi
    ret=$?
    ...
    if [ $ret -eq 124 ]; then
        ...
        head -n -1 "$outfile" > ~/tmp/func_test_${name}_expect.txt 2>/dev/null
        if [ ! -s ~/tmp/func_test_${name}_expect.txt ]; then
            > ~/tmp/func_test_${name}_expect.txt
        fi
        
        cp ~/tmp/func_test_${name}_out.txt ~/tmp/func_test_${name}_act.txt
        norm ~/tmp/func_test_${name}_act.txt
        norm ~/tmp/func_test_${name}_expect.txt
        
        if diff -q ~/tmp/func_test_${name}_act.txt ~/tmp/func_test_${name}_expect.txt > /dev/null 2>&1; then
            RUN_OK=$((RUN_OK + 1))
        else
            echo "  OUTPUT DIFF:  ${name}"
            RUN_OUTPUT_DIFF=$((RUN_OUTPUT_DIFF + 1))
            diff ~/tmp/func_test_${name}_act.txt ~/tmp/func_test_${name}_expect.txt | head -8
            echo "  ---"
        fi
    fi
done
