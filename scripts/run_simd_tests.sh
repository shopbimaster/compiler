#!/bin/bash
# Run SIMD vector syntax tests (test/functional/simd_vector/*.sy)
# Usage (in WSL Ubuntu-24.04-eval):
#   bash scripts/run_simd_tests.sh [O0|O1|o0|o1|o2|o3]
set -e
...
TEST_DIR="${PROJECT_DIR}/test/functional/simd_vector"
...
echo -e "${CYAN}============================================================${NC}"
echo -e "${CYAN}  SIMD Vector Tests (${OPT})${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""

mkdir -p /tmp/simd_vec

pass=0 fail=0
for src in "${TEST_DIR}"/*.sy; do
    [ -f "$src" ] || continue
    name=$(basename "$src" .sy)
    asm="/tmp/simd_vec/${name}.S"
    bin="/tmp/simd_vec/${name}_bin"
    cerr="/tmp/simd_vec/${name}_cerr.txt"
    lerr="/tmp/simd_vec/${name}_lerr.txt"
