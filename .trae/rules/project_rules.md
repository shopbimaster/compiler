...
| CodeSink                 | O2     | 启用 | 将指令下沉到更靠近使用者的位置；**安全约束**：LOAD 指令不可下沉（可能越过 STORE/CALL 改变内存读写顺序）；**大 BB 豁免**：BB 指令数 > 200 时跳过（避免 O(N⁴) 复杂度挂起，86_long_code2 根因）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| test21 | 2026-07 | **修复 CodeSink O(N⁴) 复杂度导致 86_long_code2 编译挂起**：CodeSink 的 `sinkInBlock` 算法在 `while(localChanged)` 外层循环中，每次移动指令后 `break` 重启，每轮扫描 O(N) 候选 × O(N) 位置查找 × O(N) 使用者查找 → 总复杂度 O(N⁴)。86_long_code2.sy 含数百项 `a[2*2][20000-1] + a[2*2][20000-1] + ...` 的巨型表达式，单 BB 有数千条指令，导致 CodeSink 无限挂起（99.9% CPU，>54s 未完成）。这是**测评服务器卡住的直接根因**。**修复**：在 `sinkInBlock` 开头添加 `if (bb->size() > 200) return false;` 大 BB 豁免。正常代码 BB 极少超过 100 条指令，200 限制安全无性能损失（CodeSink 对大 BB 收益本就有限，寄存器分配器已通过溢出处理压力）。编译时间：86_long_code2 从挂起→610ms；82_long_func 2214ms、84_long_array2 810ms、85_long_code 209ms 均正常。**本地全量 200/200 通过**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
